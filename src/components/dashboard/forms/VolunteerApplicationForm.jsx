...
const getStatusButtonStyles = (status, currentStatus) => {
  ...
  switch (status) {
    case 'Pending Review':
      specific = currentStatus === status ? `bg-ghibli-yellow text-ghibli-cream ${active} ring-ghibli-yellow-dark` : 'bg-ghibli-yellow-lightest text-ghibli-yellow-dark hover:bg-ghibli-yellow-light';
      break;
    case 'Needs More Info':
      specific = currentStatus === status ? `bg-ghibli-blue text-ghibli-cream ${active} ring-ghibli-blue-dark` : 'bg-ghibli-blue-lightest text-ghibli-blue-dark hover:bg-ghibli-blue-light';
      break;
    case 'Approved':
...
const VolunteerApplicationForm = ({ volunteer, onUpdateStatus, onCloseDrawer }) => {
  ...
  const handleUpdate = () => {
    onUpdateStatus(volunteer.id, currentStatus, adminNotes);
  };

  const avatarUrl = `https://picsum.photos/seed/${volunteer.id}/100/100`;

  return (
    <div className="flex flex-col h-full bg-ghibli-cream text-ghibli-brown">
      <div className="p-6 space-y-6 overflow-y-auto flex-grow">
        <section>
          <h3 className="text-xl font-semibold text-ghibli-dark-blue mb-4 handwritten">Applicant Information</h3>
          <div className="flex items-start space-x-4">
            ...
            <div className="space-y-1 text-sm flex-grow">
              ...
              <p><strong className="font-medium text-ghibli-brown-dark">Applied:</strong> {volunteer.applicationDate}</p>
              <p><strong className="font-medium text-ghibli-brown-dark">Volunteer ID:</strong> {volunteer.id}</p>
            </div>
          </div>
        </section>
      ...
      <div className="p-4 border-t border-ghibli-brown-light bg-ghibli-cream-light sticky bottom-0">
        <div className="flex justify-end space-x-3">
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-lg text-sm font-medium text-ghibli-cream bg-ghibli-green hover:bg-ghibli-green-dark focus:outline-none focus:ring-2 focus:ring-ghibli-green-dark focus:ring-offset-1 transition-colors duration-150"
          >
            Update Status
          </button>
        </div>
