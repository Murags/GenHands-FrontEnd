...
const IncomingDonations = () => {
  ...
  const { donations, pagination, isLoading, isError, error, refetch } = useCharityDonations(filters);

  // Filter out completed and cancelled donations to show only truly "incoming" donations
  const incomingDonations = donations.filter(donation =>
    !['cancelled', 'confirmed'].includes(donation.status)
  );
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      submitted: { color: 'bg-ghibli-yellow', text: 'Submitted', icon: ClockIcon },
      assigned: { color: 'bg-ghibli-blue', text: 'Assigned', icon: CheckCircleIcon },
      picked_up: { color: 'bg-ghibli-teal', text: 'Picked Up', icon: TruckIcon },
      delivered: { color: 'bg-ghibli-green', text: 'Delivered', icon: CheckCircleIcon },
    };
    return statusMap[status] || { color: 'bg-gray-500', text: status, icon: ClockIcon };
  };
  ...
  const getUrgencyBadge = (urgency) => {
    ...
    return urgencyMap[urgency] || { color: 'bg-gray-100 text-gray-800', text: urgency };
  };
    ...
    return (
      <div className="bg-white rounded-xl shadow-ghibli border border-ghibli-brown-light p-6">
        <div className="flex items-center justify-center py-8">
          ...
          <div>
            <p className="text-ghibli-red font-medium">Failed to load donations</p>
            <button
              onClick={() => refetch()}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-ghibli-brown mt-1">
              Track active donations from submission to delivery (excludes completed and cancelled)
            </p>
          </div>
        ...
        {showFilters && (
          <div className="mt-6 p-4 bg-ghibli-cream-lightest rounded-lg border border-ghibli-brown-light">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {/* Status Filter */}
              <div>
                <label className="block text-sm font-medium text-ghibli-dark-blue mb-1">Status</label>
                <select
              ...
              {/* Urgency Filter */}
              <div>
                <label className="block text-sm font-medium text-ghibli-dark-blue mb-1">Urgency</label>
                <select
              ...
              {/* Donor Name Search */}
              <div>
                <label className="block text-sm font-medium text-ghibli-dark-blue mb-1">Donor Name</label>
                <input
                  type="text"
              ...
              {/* Category Filter */}
              <div>
                <label className="block text-sm font-medium text-ghibli-dark-blue mb-1">Category</label>
                <input
                  type="text"
              ...
              <div className="flex items-end">
                <button
                  ...
                  className="w-full px-4 py-2 text-ghibli-brown border border-ghibli-brown-light rounded-lg hover:bg-ghibli-cream-lightest transition-colors"
                >
                  Clear Filters
                </button>
              </div>
        ...
        <div className="p-6 border-b border-ghibli-brown-light">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-ghibli-brown">
              Showing {incomingDonations.length} active donations
            </span>
          </div>
        </div>
                ...
                <div key={donation._id} className="p-6 hover:bg-ghibli-cream-lightest transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {/* Donation Items */}
                      <div className="mb-3">
                        <h5 className="font-medium text-ghibli-dark-blue mb-2">Donation Items:</h5>
                      ...
                      <div className="flex items-center space-x-2 text-sm text-ghibli-brown">
                        ...
                        <span>Submitted on {formatDate(donation.createdAt)}</span>
                      </div>
                    </div>
                    ...
                    <div className="ml-6 flex flex-col items-end space-y-2">
                      ...
                      {donation.status === 'delivered' && (
                        <div className="mt-2 px-4 py-2 bg-ghibli-red bg-opacity-10 text-ghibli-red text-sm font-medium rounded-lg flex items-center space-x-2">
